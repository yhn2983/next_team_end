...
export default function ListMaket() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3001/list-maket-edit/api/${pid}`)
      .then((r) => r.json())
      .then((result) => {
        console.log(result)
        if (result.success) {
  ...
  useEffect(() => {
    setMainSelect(formData.parent_id)
    setSubSelect(formData.category_id)
  }, [formData])
  ...
  const onSubmit = async (e) => {
    ...
    if (isNameValid && isPriceValid && isQtyValid) {
      // 資料驗證通過，可以提交表單
      try {
        const r = await fetch(`${PROD_LIST11}/${pid}`, {
          method: 'PUT',
          headers: {
                        ...
                        <td>
                          <div>
                            ...
                            >
                              ...
                              /> */}
                              {console.log(
                                'formData.product_photos',
                                formData.product_photos
                              )}
                              {/* <Image
                                src={
                                  formData.product_photos.includes(',')
                                    ? `http://localhost:3001/avatar/${formData?.product_photos
                                        .split(',')[0]
                                        .trim()}`
                                    : `http://localhost:3001/avatar/${formData?.product_photos.trim()}`
                                }
                                width={150}
                                height={150}
                                alt="Product Photo"
